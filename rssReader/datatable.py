    ...
    def nextFeed(self, order_by='-text'):
        ...
        if sib:
            ...
            while parent:
                ...
                if nextSib: 
                    return nextSib.nextFeed(order_by=order_by)
...
#             if nextSib: 
#                 return nextSib.nextUnreadFeed(order_by=order_by)
#             parent = parent.parent
...
#         return root_feed.nextUnreadFeed(order_by=order_by)
    ...
    def updateFeedData(self, parsedFeed):
        ...
        if 'title_detail' in d['feed']: 
            self.title = self.detailToTitle(d['feed']['title_detail'])
        ...
        if 'subtitle_detail' in d['feed']: 
            self.subtitle = self.detailToTitle(d['feed']['subtitle_detail'])
    ...
    def real_update(self, forced=False):
        ...
        d = feedparser.parse(self.xmlUrl, etag=self.etag, modified=mod.timetuple())
        try:
        ...
        for post in d['entries']:
            try:
                ...
                if 'contributors' in post:
                    ...
                    author += ' - '.join([self.detailToAuthor(contrib) for contrib in post['contributors']])
                    if not author:
                ...
                if 'title_detail' in post:
                    title = self.detailToTitle(post['title_detail'])
                else:
                    title = post['title']
        ...
        try:
            ...
            if sicon and sicon != 'None':
                if sicon.startswith(':/'):  # A resource name
                    icon = QtGui.QIcon(_fromUtf8(sicon))
                else:    
                ...
                icon = QtGui.QIcon(_fromUtf8(':/urssus.svg'))
            else:
            # A folder
                icon = QtGui.QIcon(_fromUtf8(':/folder.svg'))
        except:
...
def initDB():
    ...
    dbfile= 'urssus.sqlite'
    dbUrl="sqlite:///%s"%dbfile
    
    elixir.metadata.bind = dbUrl
...
#        os.system('urssus_upgrade_db')
    ...
    try:
        ...
        if root_feed is None:
            ...
            root_feed.xmlUrl = "http://cn.reuters.feedsportal.com/CNTopGenNews"
...
class MetaFeed(Feed):
    ...
    def unreadCount(self):
        if self.curUnread==-1:
            ...
            self.curUnread=Post.query.filter(eval(self.condition)).\
                filter(Post.deleted==False).\
                filter(Post.unread==True).count()
        return self.curUnread
